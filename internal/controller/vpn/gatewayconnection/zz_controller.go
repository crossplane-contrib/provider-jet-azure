...
import (
	...
	v1alpha1 "github.com/crossplane-contrib/provider-jet-azure/apis/vpn/v1alpha1"
)

// Setup adds a controller that reconciles GatewayConnection managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.GatewayConnection_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.GatewayConnection_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["azurerm_vpn_gateway_connection"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.GatewayConnection{}).
		Complete(r)
}
