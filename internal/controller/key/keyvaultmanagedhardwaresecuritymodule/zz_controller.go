...
// Setup adds a controller that reconciles KeyVaultManagedHardwareSecurityModule managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.KeyVaultManagedHardwareSecurityModuleGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.KeyVaultManagedHardwareSecurityModuleGroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.KeyVaultManagedHardwareSecurityModule{}).
		Complete(r)
}
