...
// Setup adds a controller that reconciles PostgresqlActiveDirectoryAdministrator managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.PostgresqlActiveDirectoryAdministratorGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.PostgresqlActiveDirectoryAdministratorGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_postgresql_active_directory_administrator"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.PostgresqlActiveDirectoryAdministrator{}).
		Complete(r)
}
