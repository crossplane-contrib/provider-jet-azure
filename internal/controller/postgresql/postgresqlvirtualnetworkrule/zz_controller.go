...
// Setup adds a controller that reconciles PostgresqlVirtualNetworkRule managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.PostgresqlVirtualNetworkRuleGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.PostgresqlVirtualNetworkRuleGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_postgresql_virtual_network_rule"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.PostgresqlVirtualNetworkRule{}).
		Complete(r)
}
