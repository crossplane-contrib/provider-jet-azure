...
// Setup adds a controller that reconciles PostgresqlFlexibleServerDatabase managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.PostgresqlFlexibleServerDatabaseGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.PostgresqlFlexibleServerDatabaseGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_postgresql_flexible_server_database"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.PostgresqlFlexibleServerDatabase{}).
		Complete(r)
}
