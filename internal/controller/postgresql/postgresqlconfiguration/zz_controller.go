...
// Setup adds a controller that reconciles PostgresqlConfiguration managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.PostgresqlConfigurationGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.PostgresqlConfigurationGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_postgresql_configuration"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.PostgresqlConfiguration{}).
		Complete(r)
}
