...
// Setup adds a controller that reconciles NotificationHubAuthorizationRule managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.NotificationHubAuthorizationRuleGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.NotificationHubAuthorizationRuleGroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.NotificationHubAuthorizationRule{}).
		Complete(r)
}
