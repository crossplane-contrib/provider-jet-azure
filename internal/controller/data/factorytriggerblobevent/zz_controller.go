...
package factorytriggerblobevent
...
// Setup adds a controller that reconciles FactoryTriggerBlobEvent managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.FactoryTriggerBlobEvent_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.FactoryTriggerBlobEvent_GroupVersionKind),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.FactoryTriggerBlobEvent{}).
		Complete(r)
}
