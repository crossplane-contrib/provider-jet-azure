...
import (
	...
	v1alpha1 "github.com/crossplane-contrib/provider-jet-azure/apis/containerservice/v1alpha1"
)
...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.KubernetesClusterNodePool_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.KubernetesClusterNodePool_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["azurerm_kubernetes_cluster_node_pool"],
			tjcontroller.WithCallbackProvider(tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1alpha1.KubernetesClusterNodePool_GroupVersionKind))),
		)),
