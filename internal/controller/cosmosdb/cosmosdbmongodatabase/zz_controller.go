...
// Setup adds a controller that reconciles CosmosdbMongoDatabase managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.CosmosdbMongoDatabaseGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.CosmosdbMongoDatabaseGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_cosmosdb_mongo_database"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.CosmosdbMongoDatabase{}).
		Complete(r)
}
