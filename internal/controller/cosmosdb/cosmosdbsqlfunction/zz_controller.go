...
// Setup adds a controller that reconciles CosmosdbSqlFunction managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.CosmosdbSqlFunctionGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.CosmosdbSqlFunctionGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_cosmosdb_sql_function"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.CosmosdbSqlFunction{}).
		Complete(r)
}
