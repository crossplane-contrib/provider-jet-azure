...
// Setup adds a controller that reconciles CosmosdbCassandraKeyspace managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.CosmosdbCassandraKeyspaceGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.CosmosdbCassandraKeyspaceGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("azurerm_cosmosdb_cassandra_keyspace"),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.CosmosdbCassandraKeyspace{}).
		Complete(r)
}
