...
// Setup adds a controller that reconciles StreamingEndpoint managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.StreamingEndpointGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.StreamingEndpointGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["azurerm_media_streaming_endpoint"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.StreamingEndpoint{}).
		Complete(r)
}
