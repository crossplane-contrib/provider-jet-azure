...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.ResourceGroupTemplateDeployment_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.ResourceGroupTemplateDeployment_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["azurerm_resource_group_template_deployment"],
			tjcontroller.WithCallbackProvider(tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1alpha1.ResourceGroupTemplateDeployment_GroupVersionKind))),
		)),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.ResourceGroupTemplateDeployment{}).
		Complete(r)
}
